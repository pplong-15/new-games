(function (global) {
  function normalizeQuery(raw) {
    ...
    if (!/^[A-Za-z][A-Za-z0-9]*$/.test(q)) return { ok: false, kind: "invalid" };
    return { ok: true, q: q };
  }
  ...
  function renderResults(mount, table) {
    ...
    if (!checked.ok) {
      ...
      box.innerHTML =
        "<h2>Search results</h2><p class=\"miss\">Sorry, no matching results. Search one English word at a time.</p>";
      return;
    }
    ...
    if (hit && hit.forbidden) {
      ...
      box.innerHTML =
        "<h2>Access to this file is forbidden</h2><p><span class=\"hidden-ink\">" +
    ...
    if (!hit) {
      box.innerHTML =
        "<h2>Search results</h2><p class=\"miss\">Sorry, no results related to \"" +
    ...
    box.innerHTML =
      ...
      n +
      (n === 1 ? " result matched:" : " results matched:") +
      "</p>" +
      links;
  }
