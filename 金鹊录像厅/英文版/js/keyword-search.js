(function (global) {
  ...
  function renderResults(mount, table) {
    ...
    if (!checked.ok) {
      ...
      box.innerHTML =
        "<h2>Search results</h2><p class=\"miss\">Sorry, no matching results. Search one word at a time, from a page you already opened.</p>";
      return;
    }
    ...
    if (hit && hit.forbidden) {
      ...
      box.innerHTML =
        "<h2>This file has been forbidden</h2><p><span class=\"hidden-ink\">" +
  ...
  function boot() {
    ...
    document.addEventListener("keydown", function (e) {
      ...
      var typing = /INPUT|TEXTAREA|SELECT/.test(tag) || (el && el.isContentEditable);
      if (typing) return;
      var input = document.querySelector("#search-input, input[name='q']");
      if (input) {
        e.preventDefault();
        input.focus();
      }
