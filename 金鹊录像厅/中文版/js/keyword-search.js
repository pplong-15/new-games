(function (global) {
  function normalizeQuery(raw) {
    ...
    if (!/^[\u4e00-\u9fff]+$/.test(q)) return { ok: false, kind: "invalid" };
    return { ok: true, q: q };
  }
  ...
  function renderResults(mount, table) {
    ...
    if (!checked.ok) {
      ...
      box.innerHTML =
        "<h2>搜索结果</h2><p class=\"miss\">抱歉，没有找到相关结果。每次只搜索 1 个汉字词。</p>";
      return;
    }
    ...
    if (hit && hit.forbidden) {
      ...
      box.innerHTML =
        "<h2>此文件已被禁止访问</h2><p><span class=\"hidden-ink\">" +
    ...
    if (!hit) {
      box.innerHTML =
        "<h2>搜索结果</h2><p class=\"miss\">抱歉，没有找到与\"" +
