...
try {
  ...
  ok("title-who", t.indexOf("Qiu Xiaoting") !== -1);
  ok("title-task", t.indexOf("locked group") !== -1);
  ok("title-lockline", t.indexOf("cannot still be the same person") !== -1);
  ok("title-no-hud", !/main quest|level|walkthrough|unlock|clue|ending|source code|this game|hidden page|unique solution|progress bar/.test(t));
  ...
  t = await text(page);
  ok("desk-who", t.indexOf("Anmin Lulin 6") !== -1);
  ok("desk-docs", t.indexOf("Household register") !== -1 && t.indexOf("Substitute monastic certificate") !== -1 && t.indexOf("Paper-effigy memorial") !== -1);
  ok("desk-no-approve", t.indexOf("Do not approve a deliverance rite") !== -1 && t.indexOf("Do not approve a release") !== -1);
  ok("no-approve-btn", await page.evaluate(() => {
    const btns = Array.from(document.querySelectorAll("button")).map((b) => b.textContent || "");
    return !btns.some((s) => /Approve a deliverance|Approve a release|Approve rite/.test(s));
  }));
  ...
  t = await text(page);
  ok("read-hukou", t.indexOf("still working") !== -1 && t.indexOf("1985") !== -1);
  ...
  t = await text(page);
  ok("read-dudie", t.indexOf("Jingche") !== -1 && t.indexOf("1968") !== -1);
  await page.click("#doc-dudie-fuye");
  t = await text(page);
  ok("read-fuye", t.indexOf("Zhou Shigen") !== -1);
  await page.click("#doc-shu");
  t = await text(page);
  ok("read-shu", t.indexOf("not a living person") !== -1 && t.indexOf("No fingerprint") !== -1);
  await page.click("#doc-register");
  t = await text(page);
  ok("read-register", t.indexOf("abbot does not enter") !== -1);
  await page.click("#doc-note");
  t = await text(page);
  ok("read-note", t.indexOf("not an operating guide") !== -1);
  ...
  ok("wrong-white-sms", t.indexOf("still blank") !== -1);
  ...
  ok("correct-copy", t.indexOf("Thirty-six") !== -1 || t.indexOf("¥36") !== -1);
  ok("correct-frost", await page.evaluate(() => document.getElementById("lock-table").className.indexOf("frost") !== -1));
  ok("correct-reason-merge", t.indexOf("same person") !== -1 || t.indexOf("pulled them apart") !== -1);
  ...
  ok("timeout-copy", t.indexOf("Cutoff") !== -1 && t.indexOf("¥0") !== -1);
