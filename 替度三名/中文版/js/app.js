...
(function (root) {
  ...
  function bindSelects() {
    ...
    for (i = 0; i < slots.length; i++) {
      ...
      if (nSel) {
        nSel.innerHTML = optionHtml(D.NAMES, "名分");
      ...
      if (fSel) {
        fSel.innerHTML = optionHtml(D.FATES, "怎么没的");
  ...
  function rejectText(code) {
    if (code === "cannot-name-abbot") return "住持不进名分槽。";
    if (code === "cannot-name-faname") return "法名不进户籍。";
    if (code === "court-not-in-group") return "文献，不当操作指南。不进本组。";
    if (code === "no-slot") return "先点静帧上的位置，再填这一行。";
    if (code === "hold-cancelled") return "松手了。没交。";
    if (code === "group-white") return "表还是白的。不标哪一行。";
    if (code === "gossip") return "有人说纸人会走路。别填进表。";
    if (code === "illegal-name" || code === "illegal-fate") return "这一格不收。";
    return "";
  }
  ...
  function renderDoc(state) {
    ...
    h += "</article>";
    if (doc.img) h += '<img src="jpeg/' + doc.img + '" alt="">';
    prev.innerHTML = h;
  }
  ...
  function render(state) {
    ...
    if (end) {
      if (state.endingId === "A" || state.endingId === "B") {
        ...
        var title = state.endingId === "A" ? "甲《锁定》" : "乙《未锁》";
        var pay = state.endingId === "A" ? "今晚工钱 ¥36" : "今日工钱 ¥0";
        var body = "";
        for (i = 0; i < paras.length; i++) body += "<p>" + paras[i] + "</p>";
        end.className = "ending show";
        end.innerHTML = (img ? '<img src="jpeg/' + img + '" alt="">' : "") +
          '<div class="end-copy"><h2>' + title + "</h2>" +
          body + "<p>" + pay + "</p>" +
          '<button type="button" id="btn-replay">这一晚再录一次</button></div>';
        var rp = $("btn-replay");
    ...
    if (clock) {
      ...
      var ss = sec % 60;
      clock.textContent = "距截点 " + (mm < 10 ? "0" : "") + mm + ":" + (ss < 10 ? "0" : "") + ss;
    }
    renderDoc(state);
  }
  ...
  function restoreTitle() {
    ...
    if (!TITLE_HTML) {
      TITLE_HTML = '<div id="title-layer"><img src="jpeg/00-open.jpg" alt="检查室工作台"><p class="lock-line">三份不能再是同一个人。对了才锁。错了不揭晓。</p><p class="who">邱小汀。今晚交锁定组。</p><button type="button" id="btn-enter">进站</button></div>';
    }
  ...
  function boot() {
    ...
    if (gossip) {
      gossip.onclick = function () {
        E.hearGossip();
        gossip.textContent = "有人说纸人会走路。别填进表。";
      };
    }
    E.setUi(render);
    resumeIfSaved();
