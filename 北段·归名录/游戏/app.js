...
function evidenceBody(){const p=ph(),q=p.investigation,inv=investigationState();if(!q)return '<p>本幕没有需要提交的材料判断。</p>';return `<div class="evidence-workbench"><p class="eyebrow">亲历材料 · 第 ${b().phase+1} 幕</p><h2>${esc(q.prompt)}</h2><div class="evidence-materials">${q.materials.map((m,i)=>`<article><span>材料 ${i+1}</span><h3>${esc(m.label)}</h3><p>${esc(m.text)}</p></article>`).join('')}</div><p class="evidence-rule">${inv.solved?'判断已经写入案卷。':numericReady()?'准备已齐全。哪一种解释符合眼前材料？':'你可以先读材料；'+goalStatus()+'，准备齐全后提交判断。'}</p>${inv.feedback?`<div class="evidence-feedback ${inv.solved?'solved':''}" role="status" tabindex="-1">${esc(inv.feedback)}${!inv.solved?'<p class=judgment-cost>本次误判失去 2 心神；当前心神 '+b().hp+'。</p>':''}</div>`:''}${inv.solved?`<p class="evidence-selected">你的判断：${esc(q.options.find(o=>o.id===inv.answerId)?.label||'')}</p>${button('带着判断结束回合 →','evidenceEnd','','primary')}`:`<div class="evidence-options">${q.options.map(o=>button(esc(o.label),'investigate',`data-id="${o.id}" ${!numericReady()?'disabled':''}`)).join('')}</div>`}<p class="caption">每次误判失去 2 心神；归零会失败，可从本场重试。先核对材料，再提交判断。</p></div>`;}
...
function passageBody(){const lines=passageLines(modal.kind),line=lines[modal.at]||lines[0];if(!line)return '';return `<div class="passage" style="--passage-art:url('assets/${sceneArt()}.webp')"><div class="passage-top"><span>${esc(ch().title)}</span><small>${modal.at+1} / ${lines.length}</small></div><div class="passage-line"><p class="speaker">${esc(line.speaker)}</p><p class="spoken">${esc(line.text)}</p></div><div class="passage-actions">${button('略过这段','passageSkip','','quiet')}${button(modal.at+1<lines.length?'继续听 →':modal.kind==='closing'?'整理调查结果 →':'回到现场 →','passageNext','','primary')}</div></div>`;}
...
function changes(before,after){
 ...
 if(before.phase!==after.phase)return '这一幕已通过：'+ch().phases[before.phase].title+' → '+ph().title+'。新一幕的目标进度从头计算。';
...
function renderModal(){if(!modal){$('overlay').innerHTML='';document.body.classList.remove('modal-open');return;}document.body.classList.add('modal-open');$('overlay').innerHTML=`<div class="scrim"><section class="modal ${modal==='deck'||modal==='hand'?'wide':modal?.type==='passage'?'passage-modal':''}" role="dialog" aria-modal="true" aria-label="${modal==='deck'?'整理牌组':modal==='hand'?'比较手牌':modal?.type==='investigation'?'核对现场材料':'灯下查看'}">${button('关闭 ×','close','aria-label="关闭弹窗"','close')}${modalBody()}</section></div>`;}
