...
<body>
  ...
  <footer>桐县槐溪古镇文旅　{no}/{TOTAL}</footer>
</body>
</html>
'''
...
<body>
  <table class="site" cellspacing="0" cellpadding="0">
    ...
    <tr><td colspan="2" class="ft">桐县实验剧团　办公电话不外接退票　{no}/{TOTAL}</td></tr>
  </table>
</body>
</html>
'''
...
<body>
  ...
  <div class="ft">槐溪茶馆　2008备份　{no}/{TOTAL}</div>
</body>
</html>
'''
...
<body>
  ...
  <div class="ft" style="text-align:center;font-size:12px;color:#666">个人博客　{no}/{TOTAL}</div>
</body>
</html>
'''
...
<body>
  ...
  <p style="text-align:center;font-size:12px">槐溪文献　{no}/{TOTAL}</p>
</body>
</html>
'''
...
<body>
  ...
  <p style="text-align:center;font-size:12px;color:#666">票务后台　{no}/{TOTAL}</p>
</body>
</html>
'''
...
<body>
  ...
  <p style="text-align:center;font-size:12px">抄件　{no}/{TOTAL}</p>
</body>
</html>
'''
...
def extra_copy(key: str) -> str:
    ...
    heads = [
        "窗口备忘", "值班随手记", "班车调度口", "客栈前台夹页", "茶馆旧回复",
        "文保抄写余页", "票务内部说明", "宣传科不用的段", "西厢抽屉纸条",
    ]
    people = ["老周", "小闫", "厨子家嫂子", "面的师傅", "东厢房客", "镇卫生室护士", "客运站售票", "石板街扫地的"]
    things = ["灯笼线", "井盖", "班车轮胎", "粥桶", "门牌", "香灰", "雨衣", "电池", "戏服线头", "热水阀"]
    n = 3 + (abs(hash(key)) % 40)
    who = people[abs(hash(key)) % len(people)]
    thing = things[abs(hash(key + "t")) % len(things)]
    head = heads[abs(hash(key + "h")) % len(heads)]
    a = (
        f"{head}。{who}过来说{thing}又坏了，坏了{n}天。文旅让记一笔，记了也不修。"
        f"夜戏开锣前后这两小时，镇口最乱，乱的不是鬼，是面的抢客。抢客的人认套票上的班车位，班车位经常空着，空着他们也喊。"
        f"有人把手机号写在意见本第二页，第二页会撕给剧团。撕之前谁看过，前台说记不清。"
        f"白戏那天下午人少，人少好扫地。夜戏人多，人多就把东七排的水杯踢翻。水杯不是证据，证据在簿上。"
        f"{who}让把{thing}写进交接，交接本在前台抽屉，抽屉钥匙今晚跟班车走了。"
    )
    b = (
        f"{who}还问能不能改明天。明天没有这出。这出只今晚。今晚过了，票号作废，作废不等于划名。"
        f"划名是班主的事。班主不坐窗口。窗口只念已开锣。已开锣三个字从十六点往后就可能出现，出现了别跟{who}吵。"
        f"{thing}和退票没关系。窗口仍把坏了的日子记下来，记下来给修缮组，修缮组两周来一次。"
        f"槐溪的狗晚上叫。叫跟开锣无关。有游客把狗叫写进留言，留言在茶馆，茶馆不负责狗。"
        f"雨天石板滑。滑了文旅不赔。不赔写在交通页，交通页有人懒得看，懒得看就在这儿再写一遍。"
    )
    c = (
        f"有一回{n}个人同时来问后台。后台禁客。禁客写在须知。须知文旅不链，不链是怕套票少卖。"
        f"少卖比退票省事。省事不是对游客说的话。对游客说的话是含参观。含了也不放。"
        f"乔干五点下班。下班后庙门关。关了还有人去对香火，对香火要预约，预约本页办不了。"
        f"狄厚告假这两天，上场门谁挡拍照不一定。不一定就不要拍。拍了也删，删了还在相机里，相机里的不算抄件。"
        f"抄件只认纸。纸在西厢。西厢不对外开放。不开放还把摘要挂网上，挂网上是为了少解释。"
    )
    return f"<h3>{head}</h3><p>{a}</p><p>{b}</p><p>{c}</p>"
...
<body>
  <div class="box"></div>
  <footer style="text-align:center;font-size:12px;color:#666">检索　-/{TOTAL}</footer>
</body>
</html>
'''.replace("{TOTAL}", TOTAL)
...
<body>
  ...
  <p style="text-align:center;font-size:12px">问答镜像　29/{TOTAL}</p>
</body></html>''')
