...
(function (root) {
  ...
  function fillSlot(slot, id) {
    ...
    if (slotOccupiedByLockedOther(slot, id)) {
      state.lastMessage = "Drag that one back to the bag first.";
      emit();
      return false;
    }
    var w = wordById(id);
  ...
  function submit() {
    ...
    if (!threeFull()) {
      state.lastMessage = "Incomplete. No judge.";
    ...
    if (sentenceOk()) {
      ...
      state.wage = 36;
      state.lastMessage = "Sent. The recommend field has a sentence. Staff ID is not in the signature field.";
    ...
    state.lastVerdict = "B";
    state.lastMessage = "The whole sentence does not match.";
    state.flash = false;
    state.wage = 0;
  ...
  function tryApprove() {
    ...
    state.lastMessage = "Authority stops at recommend. Cannot approve the banquet.";
    emit();
    return false;
  }
  ...
  function jumpClock(n) {
    ...
    if (n <= 0 && state.flags.ending !== "A") {
      ...
      state.lastVerdict = "B";
      state.wage = 0;
      state.lastMessage = "Eighteen. The card is empty. Deposit returned. Pay ¥0.";
