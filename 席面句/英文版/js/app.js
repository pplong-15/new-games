...
(function (root) {
  ...
  function renderDesk() {
    ...
    if (!html) html = "<span class=\"empty-bag\">Bag empty</span>";
    bag.innerHTML = html;
    ...
    ["who", "whom", "did"].forEach(function (slot) {
      ...
      var lab = { who: "Who", whom: "To whom", did: "Does what" }[slot];
    ...
    if (full && E.state.flags.ending !== "A" && !E.state.flags.timeout) {
      btn.className = "ready";
      btn.textContent = "Send to the ledger";
      btn.disabled = false;
    } else {
      btn.className = "";
      btn.textContent = full ? "Send to the ledger" : "Incomplete";
    ...
    $("clock").textContent = "Until 18:00 " + E.state.clock + " s";
