(function () {
  ...
  function renderBar(s) {
    ...
    bar.innerHTML =
      "<strong>缮写袋</strong>" +
      "<span class=\"chips\">" + (chips || "还空着") + "</span>" +
      "<span>已采 " + s.bag.length + "</span>" +
      "<a href=\"" + p + "pages/p05-desk.html\">回值班台</a>" +
      "<a href=\"" + p + "index.html\">车马店</a>";
  ...
  function wireGates(s) {
    document.querySelectorAll("a.gate[data-file]").forEach(function (a) {
      ...
      span.textContent = (a.textContent || "附件") + "（袋里的词还不够，打不开）";
  ...
  function fillSentence(s) {
    ...
    D.slotOrder.forEach(function (k) {
      ...
      if (!cell) return;
      var id = s.slots[k];
      cell.textContent = id && D.words[id] ? D.words[id].text : "（点袋里的词，再点这一栏）";
    });
  }
  ...
  function boot() {
    ...
    if (D.unlocks[file] && !canOpen(s, file)) {
      document.body.innerHTML =
        "<p class=\"gate-wait\">袋里的词还不够，这份还抽不出来。</p><p><a href=\"p05-desk.html\">回值班台</a></p>";
      renderBar(s);
      return;
    }
