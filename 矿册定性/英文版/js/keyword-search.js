(function (global) {
  function normalizeQuery(raw) {
    ...
    if (!/^([\u4e00-\u9fff]+|[A-Za-z][A-Za-z0-9]*)$/.test(q)) return { ok: false, kind: "invalid" };
    return { ok: true, q: q };
  }
  ...
  function lookup(table, q) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      for (var j = 0; j < queries.length; j++) {
        if (String(queries[j]).toLowerCase() === n) return row;
      }
    }
    return null;
  }
  ...
  function renderResults(mount, table) {
    ...
    if (!checked.ok) {
      ...
      box.innerHTML =
        "<h2>Cross-check receipt</h2><p class=\"miss\">Empty, spaces, digits, or mixed marks: this desk will not take them. Enter one token already on an opened page. No spaces.</p>";
      return;
    }
    ...
    if (hit && hit.forbidden) {
      ...
      box.innerHTML =
        "<h2>This file is not open to the temp hire</h2><p><span class=\"hidden-ink\">" +
    ...
    if (!hit) {
      box.innerHTML =
        "<h2>Cross-check receipt</h2><p class=\"miss\">No archive matching \"" +
    ...
    box.innerHTML =
      "<h2>Cross-check receipt</h2><p>Found " +
      hrefs.length +
      " matching file(s):</p>" +
      links;
  }
