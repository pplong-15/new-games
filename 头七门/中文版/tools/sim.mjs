...
E.checkDoc("flow");
ok("mutex-msg", String(E.state.lastMessage).indexOf("外人") !== -1, E.state.lastMessage);
...
ok("ghost-obit-msg", E.state.lastMessage.indexOf("不改") !== -1 || E.state.lastMessage.indexOf("只读") !== -1, E.state.lastMessage);
ok("ghost-print", E.refusePrint("吴成山") === false);
ok("ghost-print-msg", E.state.lastMessage.indexOf("手印") !== -1, E.state.lastMessage);

reset();
E.checkDoc("fang");
ok("fang-n1-closed", E.state.lastMessage.indexOf("还没纸") !== -1, E.state.lastMessage);
...
ok("slit-block", slitFail === false && E.state.lastMessage.indexOf("看不清") !== -1, E.state.lastMessage);
