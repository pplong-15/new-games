(function () {
  ...
  function searchForm(placeholder) {
    ...
    return (
      ...
      '<label class="sr-only" for="q">站内搜索</label>' +
      '<input id="q" name="q" value="' + q + '" maxlength="16" autocomplete="off" placeholder="' +
      esc(placeholder || "找人、日志、公共主页") + '">' +
  ...
  function validateQuery(raw) {
    ...
    if (!trimmed) return { ok: false, type: "empty", msg: "请输入一个词。" };
    if (/[\s\u3000]/.test(trimmed)) {
      return { ok: false, type: "multi", msg: "一次只搜一个词。空格会被当成拆开，本站没有组合检索。" };
    }
    var n = normalize(trimmed);
    if (!n) return { ok: false, type: "empty", msg: "请输入一个词。标点会被去掉。" };
    if (/[A-Za-z]/.test(trimmed) || !/[\u4e00-\u9fff]/.test(trimmed)) {
      return { ok: false, type: "en", msg: "本站索引是中文的。请用页面上出现过的汉字词，不要用英文。" };
    }
  ...
  function search(raw) {
    ...
    if (items.forbidden) {
      return { type: "forbidden", msg: "你没有权限查看该内容。", items: [], to: items.forbidTo };
    }
    ...
    if (!items.length) {
      return { type: "miss", msg: "没有找到相关结果。换一个出现在页面上的词试试。", items: [] };
    }
    return { type: "hit", msg: "共有 " + items.length + " 项结果", items: items };
  }
  ...
  function useHint() {
    ...
    state.flash = { type: "ok", text: "第 " + state.hintLevel + " 档：" + text };
    save();
    render();
  }
  ...
  function handleLogin(form) {
    ...
    var p = G.login;
    if (!user || !pass) {
      state.flash = { type: "bad", text: "请输入登录名和口令。" };
      save();
      render();
      return;
    }
    ...
    if (userOk && passOk) {
      ...
      unlockPage("inbox");
      state.flash = { type: "ok", text: "登录成功。你没有删除权，只能阅读班级公共主页里还没被清掉的东西。" };
      save();
  ...
  function chooseEnding(which) {
    ...
    if (!need.every(hasFact)) {
      state.flash = { type: "bad", text: "还不能表态。你得先把三代人和自己的位置对上。" };
      save();
      render();
      return;
    }
  ...
  function renderSearch(params) {
    ...
    document.body.innerHTML =
      '<div class="mini-bar"><div class="top-inner"><span class="logo">同窗</span><a href="home.html">首页</a>' +
      searchForm("输入一个汉字词") +
      "</div></div>" +
      '<div class="box">' +
      flashBox() +
      ...
      '<p class="muted"><a href="home.html">回首页</a></p>' +
      "</div>" +
      footerHtml(page);
    visit("search");
    bind(document.body);
  ...
  function renderPage(id) {
    var page = G.pages[id];
    if (!page) {
      document.body.innerHTML = "<p>没有这一页。</p>";
      return;
    }
