...
(function (root) {
  ...
  function openRoute(id) {
    ...
    if (!r) {
      ...
      state.lastMessage = "This path has no duty page.";
      emit();
      persist();
      return false;
    }
    ...
    if (r.skin === "fang") {
      if (state.shiftIndex === 0) state.openDoor = "fang";
        ...
        /* later shift already picked another door; prescription still readable as review */
      } else {
        state.openDoor = "fang";
      }
    }
    if (r.skin === "gui") {
      if (!firstNightDeepOk("gui") && id !== "gui-door") {
        ...
        state.lastMessage = "Tonight look at the copy they already sent open.";
        emit();
        persist();
        return false;
      }
  ...
  function writeClaim(claimId) {
    ...
    state.lastMessage = "Written: " + D.claimLabel(claimId);
    emit();
    persist();
    return true;
  }
  ...
  function handoff() {
    ...
    state.lastMessage = "Desk cleared. Proven column " + (state.verified.length ? "still here." : "has no slips.");
    emit();
    persist();
    return true;
  }
  ...
  function tryReject(kind) {
    ...
    else if (kind === "book-empty") state.lastMessage = "An empty booking calendar is not duty work.";
  ...
  function toggleClaim(id) {
    ...
    if (state.claimsTonight.length >= D.CLAIM_CAP) {
      state.lastMessage = "Three slips tonight. No more.";
      emit();
      return false;
    }
